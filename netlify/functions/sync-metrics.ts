...
export default async () => {
  ...
  if (!userToken || !userId) {
    return new Response(JSON.stringify({ ok: false, error: 'METRICOOL_USER_TOKEN / METRICOOL_USER_ID no configuradas' }), {
      status: 500,
    })
  }
  ...
  for (const brand of brands) {
    ...
    for (const post of posts) {
      const match = metricoolPosts.find((mp) => {
        const mpDate = mp.publishDate ?? mp.date
        return mpDate && post.published_at && sameDay(mpDate, post.published_at)
      })
      if (!match) continue
